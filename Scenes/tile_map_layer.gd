...
func _gui_input(event) -> void:
	...
	and event.pressed:
		...
		var to_mouse := mouse_global - self.global_position

		# Apply scale
		print("scale  -", self.scale)
		self.scale *= Vector2(zoom_factor, zoom_factor)
		# Compensate position so mouse stays fixed
		self.global_position = mouse_global - to_mouse * (scale.x / old_scale.x)
